import { adaptiveLambda, CANNOT_FUND, kellyFraction, stakeFor } from "./kelly";
 ...
 */
export function runSimulation(p: Params, onProgress?: (done: number, total: number) => void): SimResult {
  const t0 = Date.now();
  const fStar = kellyFraction(p.p, p.b);
