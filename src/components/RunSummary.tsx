...
export function RunSummary({ params, result }: { params: Params; result: SimResult | null }) {
  ...
  const forcedBelow = (lambda: number) =>
    minBet > 0 && lambda * fStar > 0 ? minBet / (lambda * fStar) : 0
  ...
  return (
    <>
      <div className="tiles">
        ...
        />
        <Tile
          k="Edge per unit staked"
          v={fmtNum(edge, 4)}
          sub="expected profit on a stake of 1"
        />
        <Tile
      ...
      {noEdge && (
        <div className="callout crit">
          ...
          <span>
            <strong>No edge at these odds.</strong> f* = {fmtNum(fStar, 4)} ≤ 0, so the Kelly-optimal
            bet is zero and both strategies sit flat for the whole run. Raise <em>p</em> or{' '}
            <em>b</em> until p·b &gt; 1 − p.
          </span>
        </div>
      )}
      ...
      {!noEdge && params.minBetEnabled && params.minBet > 0 && (
        <div className="callout warn">
          ...
          <span>
            <strong>Minimum bet is on — this is where the two rules genuinely diverge.</strong>{' '}
            (Advanced settings.) With
            continuous stakes, fractional Kelly can shrink its bet forever and never truly go broke;
            a minimum ticket of {fmtCapital(params.minBet)} breaks that. Classic is forced to
            overbet once capital falls below {fmtCapital(forcedBelow(params.lambdaConst))}; adaptive,
            betting more when small, crosses that line lower — at roughly{' '}
            {fmtCapital(forcedBelow(params.lambdaMax))}. Below the minimum ticket a path cannot bet
            at all and is counted as ruined.
          </span>
        </div>
      )}
      ...
      {!noEdge && frictionless && (
        <div className="callout">
          ...
          <span>
            <strong>Frictionless mode.</strong> No minimum bet and no ruin floor, so capital can
            halve forever without ever reaching zero — the probability of ruin will be 0% for both
            strategies by construction, no matter how reckless λ is. Turn on the minimum bet or the
            ruin floor under <em>Advanced settings</em> to see real ruin behaviour.
          </span>
        </div>
      )}
    </>
