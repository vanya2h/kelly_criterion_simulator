...
export function StatsTable({ result, theme }: { result: SimResult; theme: ChartTheme }) {
  ...
  return (
    ...
    >
      ...
      <p className="note">
        Ruined paths end at zero capital, so the growth-rate average floors their terminal capital at 10⁻⁹ × C₀ to keep
        the logarithm finite. A strategy that ruins often will show a strongly negative growth rate — that is the
        intended penalty, not an artefact.
      </p>
    </ChartCard>
  );
}
