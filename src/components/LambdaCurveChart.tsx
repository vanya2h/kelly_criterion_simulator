...
export function LambdaCurveChart({ params, theme }: { params: Params; theme: ChartTheme }) {
  ...
  const yMax =
    mode === "stake"
      ? Math.max(Math.max(params.lambdaMax, params.lambdaConst) * Math.max(fStar, 0), cap, 0.01) * 1.12
  ...
  return (
    <ChartCard
      ...
      table={{
        columns: [
          "Capital",
          mode === "stake" ? "Classic stake %" : "Classic λ",
          mode === "stake" ? "Adaptive stake %" : "Adaptive λ",
        ],
        rows: data
      ...
      <div className="chart-wrap">
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={data} margin={{ top: 20, right: 78, bottom: 22, left: 8 }}>
            ...
            />
            <YAxis domain={[0, yMax]} tickFormatter={fmtY} width={58} {...axisProps(theme)} />
            {mode === "stake" && (
              <ReferenceLine
                y={cap}
